...
class MarioAnimationDemoState extends State<MarioAnimationDemo> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    for (int i = 0; i < coinHits; i++) {
      marioYItems.add(
        TweenSequenceItem(tween: Tween(begin: 0.0, end: 1.0).chain(CurveTween(curve: Curves.easeOutQuad)), weight: wgts[2]),
      );
      marioYItems.add(
        TweenSequenceItem(tween: Tween(begin: 1.0, end: 0.0).chain(CurveTween(curve: Curves.easeIn)), weight: wgts[3]),
      );
    }
    marioYItems.add(
    ...
    marioFrame = TweenSequence<int>(marioFrameItems).animate(animationController);
    ...
    blockFrame = TweenSequence<int>(blockFrameItems).animate(animationController);
